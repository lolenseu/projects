document.addEventListener('DOMContentLoaded', function(){
  ...
  function filterRows() {
    ...
    getRows().forEach(row => {
      ...
      const idCell = row.querySelector('.teacher-id-cell');
      const nameCell = row.querySelector('.name-cell');
      if (!deptCell || !idCell || !nameCell) return;
      const dept = deptCell.textContent.trim();
      const tid = idCell.textContent.trim().toLowerCase();
      const name = nameCell.textContent.trim().toLowerCase();
      const matchesFilter = (selected === 'all' || dept === selected);
      const matchesSearch = q === '' || tid.includes(q) || name.includes(q);
  ...
  document.querySelectorAll('.edit-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (editForm) editForm.action = `/teacher/update/${id}`;
      if (editTeacherId) editTeacherId.value = btn.getAttribute('data-teacher-id') || '';
      ...
      if (editSubject) editSubject.value = btn.getAttribute('data-subject-id') || '';
  ...
  document.querySelectorAll('.view-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const viewTeacherId = document.getElementById('viewTeacherId');
      ...
      if (viewTeacherId) viewTeacherId.textContent = btn.getAttribute('data-teacher-id') || '';
      ...
      if (viewSubject) viewSubject.textContent = btn.getAttribute('data-subject') || '';
  ...
  document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (deleteForm) deleteForm.action = `/teacher/delete/${id}`;
